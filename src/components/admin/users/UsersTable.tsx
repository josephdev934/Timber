"use client";

import React, { useState } from 'react';
import { MoreHorizontal, User, Mail, Shield, Calendar, Clock, Ban, Trash2, ShieldCheck } from 'lucide-react';
import { StatusBadge } from '../shared/StatusBadge';
import { ConfirmModal } from '../shared/ConfirmModal';
import { useToast } from '../shared/useToast';
import { useAdmin } from '@/context/AdminContext';

interface UsersTableProps {
  users: any[];
  pagination: any;
  loading: boolean;
  onPageChange: (page: number) => void;
  onUserClick: (id: string) => void;
  onRefresh: () => void;
}

export function UsersTable({ users, pagination, loading, onPageChange, onUserClick, onRefresh }: UsersTableProps) {
  const { token } = useAdmin();
  const { addToast } = useToast();
  
  // Action State
  const [actionMenuId, setActionMenuId] = useState<string | null>(null);
  const [modalState, setModalState] = useState<{
    isOpen: boolean;
    type: 'ban' | 'unban' | 'delete' | null;
    userId: string | null;
    username: string | null;
    isLoading: boolean;
  }>({
    isOpen: false,
    type: null,
    userId: null,
    username: null,
    isLoading: false
  });

  const handleAction = async () => {
    const { type, userId } = modalState;
    if (!type || !userId) return;

    setModalState(s => ({ ...s, isLoading: true }));
    try {
      let endpoint = `/api/admin/users/${userId}`;
      let method = 'DELETE';
      
      if (type === 'ban') {
        endpoint += '/ban';
        method = 'PATCH';
      } else if (type === 'unban') {
        endpoint += '/unban';
        method = 'PATCH';
      }

      const res = await fetch(endpoint, {
        method,
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
        body: type === 'ban' ? JSON.stringify({ reason: 'Admin moderation' }) : undefined
      });

      if (res.ok) {
        addToast({ 
          message: `User ${type === 'ban' ? 'banned' : type === 'unban' ? 'unbanned' : 'deleted'} successfully`, 
          type: 'success' 
        });
        onRefresh();
        setModalState({ isOpen: false, type: null, userId: null, username: null, isLoading: false });
      } else {
        throw new Error("Action failed");
      }
    } catch (err) {
      addToast({ message: "Failed to perform action", type: 'error' });
    } finally {
      setModalState(s => ({ ...s, isLoading: false }));
    }
  };

  if (loading) {
    return (
      <div className="bg-[var(--color-surface)] rounded-3xl border border-[var(--color-border)] overflow-hidden animate-pulse">
        <div className="h-[400px] bg-[var(--color-elevated)]/20" />
      </div>
    );
  }

  return (
    <div className="bg-[var(--color-surface)] rounded-3xl border border-[var(--color-border)] overflow-hidden shadow-sm relative">
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-[var(--color-border)] bg-[var(--color-elevated)]/30">
              <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-[var(--color-muted)] font-sans">User</th>
              <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-[var(--color-muted)] font-sans">Email</th>
              <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-[var(--color-muted)] font-sans">Role</th>
              <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-[var(--color-muted)] font-sans">Status</th>
              <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-[var(--color-muted)] font-sans">Joined</th>
              <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-[var(--color-muted)] font-sans text-right">Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-[var(--color-border)]">
            {users.length > 0 ? users.map((user) => (
              <tr 
                key={user._id} 
                className="group hover:bg-[var(--color-elevated)]/50 transition-all duration-300 cursor-pointer"
                onClick={() => onUserClick(user._id)}
              >
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-[var(--color-bg)] border border-[var(--color-border)] flex items-center justify-center overflow-hidden shrink-0">
                      {user.profilePhoto ? (
                        <img src={user.profilePhoto} alt={user.name} className="w-full h-full object-cover" />
                      ) : (
                        <User className="w-5 h-5 text-[var(--color-muted)]" />
                      )}
                    </div>
                    <div className="min-w-0">
                      <p className="text-sm font-bold font-sans text-[var(--color-text)] truncate">{user.name}</p>
                      <p className="text-xs font-mono text-[var(--color-muted)] truncate">@{user.username}</p>
                    </div>
                  </div>
                </td>
                <td className="px-6 py-4">
                  <span className="text-sm font-mono text-[var(--color-muted)]">{user.email}</span>
                </td>
                <td className="px-6 py-4">
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-bold font-sans bg-[var(--color-elevated)] border border-[var(--color-border)] text-[var(--color-text)]">
                    {user.role}
                  </span>
                </td>
                <td className="px-6 py-4">
                  <StatusBadge status={user.isBanned ? 'banned' : 'active'} />
                </td>
                <td className="px-6 py-4">
                  <div className="flex flex-col">
                    <span className="text-sm font-mono text-[var(--color-text)]">
                      {new Date(user.createdAt).toLocaleDateString()}
                    </span>
                  </div>
                </td>
                <td className="px-6 py-4 text-right" onClick={(e) => e.stopPropagation()}>
                  <div className="relative inline-block text-left">
                    <button 
                      onClick={() => setActionMenuId(actionMenuId === user._id ? null : user._id)}
                      className="p-2 rounded-full hover:bg-[var(--color-elevated)] text-[var(--color-muted)] hover:text-[var(--color-text)] transition-all duration-300 active:scale-95"
                    >
                      <MoreHorizontal className="w-5 h-5" />
                    </button>
                    
                    {actionMenuId === user._id && (
                      <div className="absolute right-0 mt-2 w-48 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl shadow-xl z-50 p-2 animate-in fade-in zoom-in-95 duration-200">
                        {user.isBanned ? (
                          <button 
                            onClick={() => {
                              setModalState({ isOpen: true, type: 'unban', userId: user._id, username: user.username, isLoading: false });
                              setActionMenuId(null);
                            }}
                            className="w-full flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-sans text-[var(--color-text)] hover:bg-[var(--color-elevated)] transition-colors"
                          >
                            <ShieldCheck className="w-4 h-4 text-emerald-500" /> Unban User
                          </button>
                        ) : (
                          <button 
                            onClick={() => {
                              setModalState({ isOpen: true, type: 'ban', userId: user._id, username: user.username, isLoading: false });
                              setActionMenuId(null);
                            }}
                            className="w-full flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-sans text-red-500 hover:bg-red-500/5 transition-colors"
                          >
                            <Ban className="w-4 h-4" /> Ban User
                          </button>
                        )}
                        <button 
                          onClick={() => {
                            setModalState({ isOpen: true, type: 'delete', userId: user._id, username: user.username, isLoading: false });
                            setActionMenuId(null);
                          }}
                          className="w-full flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-sans text-red-600 hover:bg-red-600/5 transition-colors"
                        >
                          <Trash2 className="w-4 h-4" /> Delete User
                        </button>
                      </div>
                    )}
                  </div>
                </td>
              </tr>
            )) : (
              <tr>
                <td colSpan={6} className="px-6 py-20 text-center text-[var(--color-muted)] font-sans italic">
                  No users found matching your criteria
                </td>
              </tr>
            )}
          </tbody>
        </table>
      </div>

      {/* Pagination Bar */}
      <div className="px-6 py-4 bg-[var(--color-elevated)]/20 border-t border-[var(--color-border)] flex items-center justify-between">
        <p className="text-xs font-sans text-[var(--color-muted)]">
          Page <span className="font-mono text-[var(--color-text)] font-bold">{pagination.page}</span> of <span className="font-mono text-[var(--color-text)] font-bold">{pagination.totalPages}</span>
        </p>
        <div className="flex items-center gap-2">
          <button 
            onClick={() => onPageChange(pagination.page - 1)}
            disabled={pagination.page <= 1}
            className="px-4 py-2 rounded-full text-xs font-sans font-medium text-[var(--color-text)] border border-[var(--color-border)] disabled:opacity-30 hover:bg-[var(--color-elevated)] transition-all active:scale-95"
          >
            Previous
          </button>
          <button 
            onClick={() => onPageChange(pagination.page + 1)}
            disabled={pagination.page >= pagination.totalPages}
            className="px-4 py-2 rounded-full text-xs font-sans font-medium text-[var(--color-text)] border border-[var(--color-border)] disabled:opacity-30 hover:bg-[var(--color-elevated)] transition-all active:scale-95"
          >
            Next
          </button>
        </div>
      </div>

      {/* MODALS */}
      <ConfirmModal 
        isOpen={modalState.isOpen}
        title={`${modalState.type?.toUpperCase()} USER`}
        description={`Are you sure you want to ${modalState.type} ${modalState.username}? This action will be logged.`}
        requireTypedConfirm={modalState.type === 'delete'}
        confirmWord="CONFIRM"
        isLoading={modalState.isLoading}
        onConfirm={handleAction}
        onCancel={() => setModalState({ isOpen: false, type: null, userId: null, username: null, isLoading: false })}
      />
    </div>
  );
}
